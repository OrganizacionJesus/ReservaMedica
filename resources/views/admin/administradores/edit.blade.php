...
@php
    ...
    $bannerColor = $admin->banner_color ?? 'bg-gradient-to-br from-orange-500 via-amber-600 to-yellow-600';
...
<div class="relative overflow-hidden rounded-[2rem] {{ $baseColorClass }} shadow-2xl mb-8" 
    ...
    @if(!$baseColorClass && !$baseColorStyle)
        <div class="absolute inset-0 bg-gradient-to-br from-orange-500 via-amber-600 to-yellow-600"></div>
    @endif
    ...
    <div class="relative z-10 p-6 flex items-center gap-4">
        ...
        <div class="text-white" style="color: var(--text-on-medical, #ffffff);">
            ...
            <p class="text-white/90 text-sm font-medium" style="opacity: 0.95;">
                Actualiza los datos de {{ $administrador->primer_nombre }} {{ $administrador->primer_apellido }}
            </p>
        </div>
    </div>
</div>
...
<form id="editAdminForm" method="POST" action="{{ route('administradores.update', $administrador->id) }}">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <select name="tipo_documento" id="tipo_documento"
                                        ...
                                        required>
                                    <option value="V" {{ old('tipo_documento', $administrador->tipo_documento) == 'V' ? 'selected' : '' }}>V</option>
                                    <option value="E" {{ old('tipo_documento', $administrador->tipo_documento) == 'E' ? 'selected' : '' }}>E</option>
                                    <option value="P" {{ old('tipo_documento', $administrador->tipo_documento) == 'P' ? 'selected' : '' }}>P</option>
                                    <option value="J" {{ old('tipo_documento', $administrador->tipo_documento) == 'J' ? 'selected' : '' }}>J</option>
                                </select>
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <select name="prefijo_tlf" id="prefijo_tlf"
                                        class="w-24 px-4 py-3 bg-white/70 backdrop-blur-sm border-2 border-gray-200/60 rounded-xl focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all font-bold" 
                                        required>
                                    <option value="+58" {{ old('prefijo_tlf', $administrador->prefijo_tlf) == '+58' ? 'selected' : '' }}>+58</option>
                                    <option value="+57" {{ old('prefijo_tlf', $administrador->prefijo_tlf) == '+57' ? 'selected' : '' }}>+57</option>
                                    <option value="+1" {{ old('prefijo_tlf', $administrador->prefijo_tlf) == '+1' ? 'selected' : '' }}>+1</option>
                                </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="ciudad_id" id="ciudad_id"
                                    class="w-full px-4 py-3 bg-white/70 backdrop-blur-sm border-2 border-gray-200/60 rounded-xl focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all font-medium" 
                                    {{ old('estado_id', $administrador->estado_id) ? '' : 'disabled' }}>
                        ...
                        <div>
                            ...
                            <select name="parroquia_id" id="parroquia_id"
                                    class="w-full px-4 py-3 bg-white/70 backdrop-blur-sm border-2 border-gray-200/60 rounded-xl focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 transition-all font-medium" 
                                    {{ old('municipio_id', $administrador->municipio_id) ? '' : 'disabled' }}>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="direccion_detallada" id="direccion_detallada"
                                      ...
                                      rows="2">{{ old('direccion_detallada', $administrador->direccion_detallada) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="p-4 bg-gradient-to-r from-yellow-50 to-amber-50 border-2 border-yellow-200 rounded-2xl mb-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-yellow-700 font-medium">Deje los campos en blanco si no desea cambiarla.</p>
                                </div>
                            </div>
                        </div>
        ...
        <div class="space-y-6">
                ...
                <div class="relative bg-white/60 backdrop-blur-2xl rounded-[2rem] border border-white/80 shadow-2xl p-6">
                    ...
                    <label class="flex items-center gap-3 p-4 border-2 border-gray-200/60 rounded-2xl cursor-pointer hover:bg-white/70 transition-all">
                        <input type="checkbox" name="status" value="1" 
                               class="w-5 h-5 text-emerald-600 focus:ring-emerald-500 focus:ring-4 rounded" 
            ...
            <!-- Actions -->
            <div class="relative group">
                <div class="absolute -inset-0.5 bg-gradient-to-r from-orange-300 via-amber-300 to-yellow-300 rounded-[2rem] opacity-0 group-hover:opacity-20 blur-2xl transition-all duration-500"></div>
                <div class="relative bg-white/60 backdrop-blur-2xl rounded-[2rem] border border-white/80 shadow-2xl p-6">
                    <h3 class="text-lg font-black text-gray-900 mb-4">Acciones</h3>
                    <div class="space-y-3">
                        <button type="submit" 
                                class="w-full px-6 py-4 bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white font-black rounded-2xl shadow-lg hover:shadow-xl hover:scale-105 transition-all flex items-center justify-center gap-2">
...
document.addEventListener('DOMContentLoaded', function() {
    const initialData = {
        estado_id: "{{ old('estado_id', $administrador->estado_id ?? '') }}",
        ciudad_id: "{{ old('ciudad_id', $administrador->ciudad_id ?? '') }}",
        municipio_id: "{{ old('municipio_id', $administrador->municipio_id ?? '') }}",
        parroquia_id: "{{ old('parroquia_id', $administrador->parroquia_id ?? '') }}"
    };
    ...
    async function loadDropdowns(type, parentId, selected Id = null) {
        if (!parentId) return;
        try {
            let pluralType = type + 's';
            if (type === 'ciudad') pluralType = 'ciudades';
            
            const response = await fetch(`{{ url('admin/get-${pluralType}') }}/${parentId}`.replace('${pluralType}', pluralType));
